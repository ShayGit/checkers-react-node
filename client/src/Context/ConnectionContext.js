...
const connReducer = (state, action) => {
  ...
  const createRoom = (dispatch) => async (username) => {
    try {
        const roomId = uuid();
        dispatch({ type: "set_connection_info", payload: {username,roomId,isPlayer1:true} });
  ...
  const joinRoom = (dispatch) => async (username,roomid) => {
    try {
      dispatch({ type: "set_connection_info", payload: {username,roomId:roomid,isPlayer1:false} });
    } catch (error) {
      alert(error.message);
    }
  };
